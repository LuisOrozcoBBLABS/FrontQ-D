...
| Rama | Qué cambió |
|---|---|
| `QA` | **La API de producción se mudó.** `apiUrl` pasa de `backq-d.onrender.com` a `backq-d-y6sq.onrender.com`, y el `preconnect` de `index.html` con ella — seguía adelantando DNS y TLS al servidor viejo, así que la optimización no servía y encima despertaba la instancia que queremos dejar morir. **La instancia anterior sigue viva y no se puede apagar:** quedó bajo una cuenta personal a la que la empresa ya no tiene acceso. Sus datos están congelados en la foto que se migró. Volver a apuntar el front allá haría que la plataforma pareciera funcionar mientras muestra datos viejos y escribe en una base que nadie controla, sin ningún síntoma visible — por eso la advertencia quedó escrita en `environment.prod.ts`. |
| `QA` | **Paginar dejaba de cambiarte el proyecto que estabas leyendo.** En la vista maestro-detalle, pasar de la página 1 a la 2 mostraba otro proyecto sin que nadie lo eligiera. La causa estaba en una sola línea de `ui/seleccion-maestro`: `items.find(...) ?? items[0]`, que mezclaba dos situaciones distintas — «nadie eligió todavía», donde caer al primero es correcto, y «eligió, y lo elegido no está en esta página», donde caer al primero le cambia la pantalla a alguien que no pidió nada. Ahora la selección guarda además la última versión conocida de lo elegido, para poder seguir mostrándolo cuando sale de la página; la lista sigue ganando cuando lo contiene, así que una edición se ve igual. El helper lo comparten proyectos y usuarios, así que el arreglo cubre los dos. 18 tests nuevos, validados reintroduciendo el bug: seis fallan. |
| `QA` | **Campo cliente y rol comercial.** El input entra una sola vez, en `project-fields`, que es el componente compartido por los tres formularios; se muestra en la tarjeta del tablero y en la cabecera del detalle, con el acento para separarlo del sector, que es la etiqueta de al lado. En la tarjeta se recorta con puntos suspensivos: un nombre de empresa largo no puede empujar la fila. El rol `comercial` **no necesitó ni un bloqueo nuevo** — las rutas de creación ya están detrás de `permissionGuard('projects.create')` y editar/archivar detrás de `esAutorOAdmin()`, y un comercial no es ninguna de las dos cosas. Lo que sí hubo que arreglar son tres lugares que asumían dos roles: la ficha de usuario rotulaba a un comercial como «Colaborador», el filtro de roles se había quedado corto, y `/proyectos` le daba el tablero de «lo mío», que le saldría **vacío** porque no recibe asignaciones. Ahora hay un `ROL_LABEL: Record<RoleId, string>` que hace que agregar un rol sin nombrarlo no compile. |
| `QA` | **Tarjeta de cristal en el ingreso, y la marca a la izquierda del nombre.** Mismo material del riel. Conviene saber por qué acá sí y en el panel de avisos no, porque ese lleva el comentario contrario: allá el panel vive dentro de un ancestro con `backdrop-filter`, que establece un Backdrop Root y le recorta lo que puede muestrear a una caja de 64px de la que el panel sale entero. Acá no hay ese ancestro y lo de atrás es conocido y estático, así que el contraste se puede acotar: medido componiendo el cristal sobre la página, 17.17:1 en oscuro y 19.80:1 en claro. El lockup horizontal obligó a bajar el logotipo a 36px y el título a `--fs-xl` — a los tamaños anteriores pedía 417px de ancho útil contra los 356 disponibles y «Plataforma R&D» se partía en dos renglones. |
| `QA` | **Logotipo más grande, en las cuatro pantallas de acceso, y confirmación al cerrar sesión.** El de la tira pasa de 40 a 56px: a 40 medía 118px de ancho en una tira de 1500, que es una marca de agua y no una firma. Se le quitó también la opacidad del 92%, porque atenuar lo que se quiere que se note trabaja en contra. Las pantallas de acceso cambian el isotipo recortado por el logotipo completo, y el intercambio por tema tuvo que mudarse a `styles.scss`: una regla `html[data-theme="..."]` dentro de un estilo de componente **nunca** matchea, porque la encapsulación le agrega el atributo de scope al propio `<html>`. Un detalle que solo apareció midiendo: en la columna del acceso el logotipo salía 372×44, o sea aplastado — un flex en columna estira sus hijos y contra eso `width:auto` no gana. Cerrar sesión ahora pregunta, con `danger:false` a propósito: no destruye nada, y si se gasta el rojo acá deja de avisar cuando importa. |
| `QA` | **El diálogo destructivo tenía los dos botones sin contraste**, no uno. Reportado como «el cancelar no es visible» al archivar un grupo; medido, eran «Cancelar» en 1.43:1 y «Archivar» en 2.77:1, y solo en tema claro. El primero salía como botón de solo texto y tomaba el color primario del tema —el lima de marca—, que sobre un diálogo claro es casi invisible. El segundo fallaba por otra razón: el preset no mapeaba las severidades de PrimeNG y usaba el rojo por defecto de la librería. Apareció además un tercer defecto al verificar el arreglo: el borde del botón contorneado iba en `--line-strong`, color de hairline, 1.45:1 — un contorno que no se ve no es un contorno. Los ocho valores pasan AA ahora, mínimo 5.26:1. |
